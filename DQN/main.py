...
# Menu options
menu_options = ["Start Game", "DQN Agent Mode", "Rules", "Exit"]
...
def game_loop():
    ...
    while running:
        ...
        if not game_over:
            ...
            # Live training using Behavioral Cloning
            dqn_agent.add_to_replay_buffer(current_state, action, new_state, reward, terminated)
...
def dqn_agent_mode():
    ...
    while running:
        ...
        if not game_over:
            ...
            # Get current state from ray casting
            ray_dist = ray_cast(playerX, playerY, angle)
            
            # Get action from the actual DQN agent
            action = dqn_agent.get_action_from_model(ray_dist)

            accelerating = False
            braking = False

            # Determine keys based on DQN action
            if action in [Action.UP, Action.UP_LEFT, Action.UP_RIGHT]:
                ...
                if player_speed < 0:
                    ...
                    if player_speed > max_speed:
                        player_speed = max_speed
            elif action in [Action.DOWN, Action.DOWN_LEFT, Action.DOWN_RIGHT]:
                braking = True
                if player_speed > 0:
                    player_speed -= friction
                else:
            ...
            if player_speed != 0:  # Rotation only when moving
                direction = 1 if player_speed > 0 else -1
            ...
            if action in [Action.LEFT, Action.UP_LEFT, Action.DOWN_LEFT]:
                steering_angle = max(steering_angle - steering_sensitivity, -30)
            elif action in [Action.RIGHT, Action.UP_RIGHT, Action.DOWN_RIGHT]:
                steering_angle = min(steering_angle + steering_sensitivity, 30)
            else:
                if steering_angle > 0:
                    steering_angle = max(steering_angle - steering_return_speed, 0)
            ...
            handle_steering_wheel(steering_angle, player_speed, "dqn")
            draw_pedals(accelerating, braking)
